...
const ProfileCard = dynamic(() => import('ethereum-identity-kit').then((mod) => mod.ProfileCard), { ssr: false })
```
...
export default function Profile() {
  return <ClientOnly fallback={<div>Loading...</div>}>{() => <ProfileCard addressOrName="vitalik.eth" />}</ClientOnly>
}
```
