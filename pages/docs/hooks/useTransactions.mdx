...
**Description**:  
A function to add a list operations transaction. This is handled by the Follow Button component, however you can use it to add a list operations transaction manually.
...
addListOpsTransaction({ id: 'listOp1', ... });
```
...
```tsx
removeListOpsTransaction(['0x123', '0x456'])
```
