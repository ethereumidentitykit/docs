...
;<FollowersYouKnow lookupAddress="vitalik.eth" showEmpty={false} showLoading={true} />
```
...
function useTypedProfile<T extends string | `0x${string}`>(addressOrName: T): UseProfileDetailsReturn {
...
const CustomProfileCard = forwardRef<HTMLDivElement, React.ComponentProps<typeof ProfileCard>>((props, ref) => {
